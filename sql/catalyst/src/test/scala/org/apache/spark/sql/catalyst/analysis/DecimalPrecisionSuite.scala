...
import org.apache.spark.sql.catalyst.catalog.{InMemoryCatalog, InternalCatalog, SessionCatalog}
...
class DecimalPrecisionSuite extends PlanTest with BeforeAndAfter {
  ...
  private val catalog = new SessionCatalog(new InternalCatalog(new InMemoryCatalog),
