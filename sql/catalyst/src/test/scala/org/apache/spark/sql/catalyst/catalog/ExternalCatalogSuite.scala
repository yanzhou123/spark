...
abstract class ExternalCatalogSuite extends SparkFunSuite with BeforeAndAfterEach {
  ...
  test("drop database when the database is not empty") {
    ...
    catalog1.dropTable(TableIdentifier("tbl1", Some("db2")), ignoreIfNotExists = false)
    catalog1.dropTable(TableIdentifier("tbl2", Some("db2")), ignoreIfNotExists = false)
  ...
  test("the table type of an external table should be EXTERNAL_TABLE") {
    ...
    val actual = catalog.getTable(TableIdentifier("external_table1", Some("db2")))
  ...
  test("drop table when database/table does not exist") {
    ...
    intercept[AnalysisException] {
      catalog.dropTable(TableIdentifier("unknown_table", Some("unknown_db")),
  ...
  test("rename table") {
    val catalog = newBasicCatalog()
    assert(catalog.listTables("db2").toSet == Set("tbl1", "tbl2"))
    catalog.renameTable(TableIdentifier("tbl1", Some("db2")), "tblone")
  ...
  test("rename table when database/table does not exist") {
    val catalog = newBasicCatalog()
    intercept[AnalysisException] {
      catalog.renameTable(TableIdentifier("unknown_table", Some("unknown_db")), "unknown_table")
    }
    intercept[AnalysisException] {
      catalog.renameTable(TableIdentifier("unknown_table", Some("db2")), "unknown_table")
    }
  }
  ...
  test("rename table when destination table already exists") {
    val catalog = newBasicCatalog()
    intercept[AnalysisException] {
      catalog.renameTable(TableIdentifier("tbl1", Some("db2")), "tbl2")
    }
  }

  test("alter table") {
    val catalog = newBasicCatalog()
    val tbl1 = catalog.getTable(TableIdentifier("tbl1", Some("db2")))
    catalog.alterTable(tbl1.copy(properties = Map("toh" -> "frem")))
    val newTbl1 = catalog.getTable(TableIdentifier("tbl1", Some("db2")))
  ...
  test("get table") {
    assert(newBasicCatalog().getTable(
      TableIdentifier("tbl1", Some("db2"))).identifier.table == "tbl1")
  }

  test("get table when database/table does not exist") {
    val catalog = newBasicCatalog()
    intercept[AnalysisException] {
      catalog.getTable(TableIdentifier("unknown_table", Some("unknown_db")))
    }
    intercept[AnalysisException] {
  ...
  test("basic create and list partitions") {
    ...
    catalog.createPartitions(TableIdentifier("tbl", Some("mydb")),
  ...
  test("create partitions that already exist") {
    ...
    intercept[AnalysisException] {
      catalog.createPartitions(TableIdentifier("tbl2", Some("db2")),
        Seq(part1), ignoreIfExists = false)
    }
    catalog.createPartitions(TableIdentifier("tbl2", Some("db2")),
      Seq(part1), ignoreIfExists = true)
  }
  ...
  test("drop partitions") {
    ...
    assert(catalog2.listPartitions(TableIdentifier("tbl2", Some("db2"))).isEmpty)
  }
  ...
  test("get partition") {
    ...
    intercept[AnalysisException] {
      catalog.getPartition(TableIdentifier("tbl1", Some("db2")), part3.spec)
    }
  }
  ...
  test("rename partitions") {
    ...
    intercept[AnalysisException] { catalog.getPartition(
      TableIdentifier("tbl2", Some("db2")), part1.spec) }
    intercept[AnalysisException] { catalog.getPartition(
      TableIdentifier("tbl2", Some("db2")), part2.spec) }
  }
  ...
  test("rename partitions when the new partition already exists") {
    ...
    intercept[AnalysisException] {
      catalog.renamePartitions(
        TableIdentifier("tbl2", Some("db2")), Seq(part1.spec), Seq(part2.spec))
    }
  }
  ...
  test("alter partitions") {
    ...
    try {
      ...
      val newPart1b = catalog.getPartition(TableIdentifier("tbl2", Some("db2")), part1.spec)
      val newPart2b = catalog.getPartition(TableIdentifier("tbl2", Some("db2")), part2.spec)
  ...
  test("create/drop/rename table should create/delete/rename the directory") {
    ...
    catalog.renameTable(TableIdentifier("my_table", Some("db1")), "your_table")
    assert(!exists(db.locationUri, "my_table"))
    assert(exists(db.locationUri, "your_table"))

    catalog.dropTable(TableIdentifier("your_table", Some("db1")), ignoreIfNotExists = false)
    assert(!exists(db.locationUri, "your_table"))
  ...
  test("create/drop/rename partitions should create/delete/rename the directory") {
    ...
    catalog.createPartitions(
      TableIdentifier("tbl", Some("db1")), Seq(part1, part2), ignoreIfExists = false)
    assert(exists(databaseDir, "tbl", "a=1", "b=2"))
    assert(exists(databaseDir, "tbl", "a=3", "b=4"))

    catalog.renamePartitions(
      TableIdentifier("tbl", Some("db1")), Seq(part1.spec), Seq(part3.spec))
    assert(!exists(databaseDir, "tbl", "a=1", "b=2"))
    assert(exists(databaseDir, "tbl", "a=5", "b=6"))

    catalog.dropPartitions(
      TableIdentifier("tbl", Some("db1")), Seq(part2.spec, part3.spec), ignoreIfNotExists = false)
    assert(!exists(databaseDir, "tbl", "a=3", "b=4"))
    assert(!exists(databaseDir, "tbl", "a=5", "b=6"))
    ...
    catalog.createPartitions(
      TableIdentifier("tbl", Some("db1")), Seq(externalPartition), ignoreIfExists = false)
...
abstract class CatalogTestUtils {
  ...
  def catalogPartitionsEqual(
    ...
    catalog.listPartitions(
      TableIdentifier(table, Some(db))).map(_.spec).toSet == parts.map(_.spec).toSet
  }
